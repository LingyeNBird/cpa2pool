...
func (r *Runtime) Register(raw []byte) (any, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	var req struct {
	...
	if cfg.Database == "" {
		cfg.Database = "data/cpa2pool.db"
	}
	if r.db != nil && r.path != cfg.Database {
		return nil, errors.New("修改数据库路径需要重启 CPA")
	}
	if r.db == nil {
...
func (r *Runtime) Handle(method string, raw []byte) (any, error) {
	...
	case "management.register":
		return map[string]any{"routes": api.Routes(), "resources": []map[string]string{{"Path": "/ui", "Menu": "拼车额度"}}}, nil
	...
	case "request.intercept_before":
		...
		return intercept(r.billing.Before(req.RequestID, scope, model, req.Body))
	case "request.intercept_after":
		return intercept(r.billing.After(req.RequestID, req.Model))
	case "response.intercept_after":
		return map[string]any{}, r.billing.Response(req.RequestID, req.Body, false)
	case "response.intercept_stream_chunk":
		return map[string]any{}, r.billing.Response(req.RequestID, req.Body, true)
	case "websocket.response_event":
		if req.EventType == "response.completed" || req.EventType == "response.done" {
			return map[string]any{}, r.billing.Response(req.RequestID, req.Payload, false)
		}
		return map[string]any{}, nil
...
func intercept(err error) (any, error) {
	if err == nil {
		return map[string]any{}, nil
	}
	code := 503
	reason := "billing_unavailable"
	var denial *billing.Denial
	if errors.As(err, &denial) {
		code = denial.Status
		reason = denial.Reason
	}
	return map[string]any{"Terminate": true, "StatusCode": code, "ResponseHeaders": http.Header{"Content-Type": {"application/json"}}, "ResponseBody": []byte(store.JSON(map[string]any{"error": map[string]any{"type": "cpa2pool_rejected", "message": reason}}))}, nil
}
