...
func TestAnthropicStreamAccumulatesInputAndFinalOutput(t *testing.T) {
	...
	m.Flush()
	if m.Usage.Input != 10 || m.Usage.Output != 50 || m.Usage.CacheRead != 20 || m.Usage.CacheWrite != 30 {
		t.Fatalf("merged usage: %+v", m.Usage)
	}
}
func TestOpenAIReasoningIsNotBilledTwice(t *testing.T) {
	var m Meter
	m.JSON([]byte(`{"usage":{"input_tokens":100,"input_tokens_details":{"cached_tokens":40},"output_tokens":20,"output_tokens_details":{"reasoning_tokens":15}}}`))
	if m.Usage.Input != 60 || m.Usage.CacheRead != 40 || m.Usage.Output != 20 || m.Usage.Reasoning != 15 {
...
func TestCPAStreamLinesWithoutNewlines(t *testing.T) {
	...
	m.Flush()
	if !m.Seen || m.Usage.Input != 100 || m.Usage.Output != 20 {
		t.Fatalf("undelimited CPA frames lost usage: %+v", m)
	}
}
