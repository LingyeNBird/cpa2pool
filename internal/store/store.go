...
func One[T any](q Query, query string, args ...any) (T, error) {
	var v T
	var b string
	err := q.QueryRow(query, args...).Scan(&b)
	if err != nil {
		return v, err
	}
...
func List[T any](q Query, query string, args ...any) ([]T, error) {
...
func Audit(q Query, pid, qid, action, note string, before, after any) error {
	a := domain.Audit{ID: domain.ID(), ParticipantID: pid, QuotaID: qid, Action: action, Note: note, Time: domain.Now(), Before: before, After: after}
	_, err := q.Exec("INSERT INTO audits VALUES(?,?,?,?,?)", a.ID, pid, qid, a.Time.Format("2006-01-02T15:04:05.000000000Z"), JSON(a))
	return err
}
