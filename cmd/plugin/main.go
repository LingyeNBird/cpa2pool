...
func cliproxy_plugin_init(host *C.cliproxy_host_api,p *C.cliproxy_plugin_api)C.int{
 p.abi_version=1;p.call=C.cliproxy_plugin_call_fn(C.cliproxyPluginCall);p.free_buffer=C.cliproxy_plugin_free_fn(C.cliproxyPluginFree);p.shutdown=C.cliproxy_plugin_shutdown_fn(C.cliproxyPluginShutdown);return 0
}
...
func cliproxyPluginCall(method *C.char,request *C.uint8_t,length C.size_t,response *C.cliproxy_buffer)C.int{
 ...
 if err!=nil{envelope=map[string]any{"ok":false,"error":map[string]string{"code":"plugin_error","message":err.Error()}}}
 raw,marshalErr:=json.Marshal(envelope);if marshalErr!=nil{return 1};response.ptr=C.CBytes(raw);response.len=C.size_t(len(raw));return 0
}
